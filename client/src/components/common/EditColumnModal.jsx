  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-black rounded-2xl p-6 w-full max-w-md shadow-lg">
        <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-gray-100">Edit Column</h2>
              ...
              {Object.entries(colors).map(([key, col]) => (
                <button
                  ...
                  className={`w-8 h-8 rounded-full border-2 ${
                    colorKey === key ? "border-black dark:border-white" : "border-transparent"
                  }`}
                  style={{ backgroundColor: isDarkMode ? col.dark.fill : col.light.fill }}
                />
              ))}
            </div>
          </div>
